...
public class KeywordsPredictHandler extends AbstractPredictHandler {
    ...
    public MatchResult predictFlow(String content,  TemplateNodeInfo flowNode,  DialogueProcessSession session) {
        ...
        log.info("callId:{},contentMasked:{},进入关键词预测 ", callId, ProcessChatLogFormatter.maskUtterance(content));
        //根据节点node 所有意图分支升序排序
        ...
        log.info("callId:{},contentMasked:{},关键词匹配结果:{}", callId, ProcessChatLogFormatter.maskUtterance(content), set);
        ...
        if (!set.isEmpty()) {
            for (TemplateNodeBranchInfo branch : branches) {
                if (set.contains(branch.intentionId())) {
                    ...
                    //匹配成功
                    log.info("callId:{},contentMasked:{},关键词匹配成功,进入节点:{}", callId, ProcessChatLogFormatter.maskUtterance(content), branch.targetFlowId());
        ...
        for (TemplateNodeBranchInfo branch : branches) {
            ...
            for (Keyword keyword : keywords) {
                if (matchLogic(keyword, content)) {
                    //匹配成功
                    buildSessionIntentionClassify(session,branch.classify());
                    log.info("callId:{},contentMasked:{},expressionLen:{},进入节点:{}", callId,
                            ProcessChatLogFormatter.maskUtterance(content),
                            keyword.value() == null ? 0 : keyword.value().length(), branch.targetFlowId());
