                ...
                if (q != null) {
                    ...
                    switch (q.type()) {
                        ...
                        case BETWEEN:
                            ...
                            if(between.size() == 2){
                                list.add(cb.between(getExpression(attributeName, join, root).as((Class<? extends Comparable>) between.getFirst().getClass()),
                                        (Comparable) between.getFirst(), (Comparable) between.get(1)));
                            }
                            break;
                        case FIND_IN_SET:
                            ...
                            break;
                        case EQUAL_OR_NULL:
                            list.add(cb.or(cb.equal(getExpression(attributeName, join, root), val), cb.isNull(getExpression(attributeName, join, root))));
                            break;
                        case NOT_EQUAL_OR_NULL:
                            list.add(cb.or(cb.notEqual(getExpression(attributeName, join, root), val), cb.isNull(getExpression(attributeName, join, root))));
                            break;
                        default: break;
                    }
                }
