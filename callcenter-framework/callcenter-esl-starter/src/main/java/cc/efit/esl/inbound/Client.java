...
import cc.efit.esl.core.*;
import cc.efit.esl.core.event.EslEvent;
import cc.efit.esl.core.exception.InboundClientException;
 ...
 */
public abstract class Client implements IModEslApi {
    ...
    protected  abstract void onChannelDisconnect();
	 ...
	 */
	public void connect(SocketAddress clientAddress, String password, int timeoutSeconds) throws InboundClientException {
		...
		if (!future.awaitUninterruptibly(timeoutSeconds, TimeUnit.SECONDS)) {
			throw new InboundClientException("Timeout connecting to " + clientAddress);
		}
		// Did not timeout
		...
		if (!future.isSuccess()) {
			...
			throw new InboundClientException("Could not connect to " + clientAddress, future.cause());
		}
		...
		if (!authenticated) {
			throw new InboundClientException("Authentication failed: " + authenticationResponse.getReplyText());
		}
	...
	public void close() {
        ...
        if (clientContext.isPresent()) {
            new CommandResponse("exit", clientContext.get().sendCommand("exit"));
        } else {
