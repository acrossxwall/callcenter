...
public class Context implements IModEslApi {
	 ...
	 * server.
	 * <p/>
	 * The outcome of the command from the server is returned in an {@link cc.efit.esl.core.message.EslMessage} object.
	 *
	 * @param command a mod_event_socket command to send
	 * @return an {@link cc.efit.esl.core.message.EslMessage} containing command results
	 */
	public EslMessage sendCommand(String command) {
		checkArgument(command!=null, "command cannot be null or empty");
	 ...
	 * @return a {@link cc.efit.esl.core.res.CommandResponse} with the server's response.
	 */
	@Override
	public CommandResponse setEventSubscriptions(EventFormat format, String events) {
		checkArgument(format.equals(EventFormat.PLAIN), "Only 'plain' event format is supported at present");
        final StringBuilder sb = new StringBuilder();
	...
	public CommandResponse addEventFilter(String eventHeader, String valueToFilter) {
		checkArgument(eventHeader!=null, "eventHeader cannot be null or empty");
        final StringBuilder sb = new StringBuilder();
        sb.append("filter ").append(eventHeader);
        if (!Objects.isNull(valueToFilter)) {
	...
	public CommandResponse sendMessage(SendMsg sendMsg) {
		checkArgument(sendMsg!=null, "sendMsg cannot be null");
