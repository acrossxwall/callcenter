...
@Service
@Slf4j
public class NegativeCountRule implements RuleHandler {
    @Override
    public boolean handleRuleMatch(RuleInfo rule, DialogueProcessSession session, List<InteractiveRecord> list, int callDuration) {
        ...
        log.info("callId: 触发否定次数规则匹配{}",callId);
        return ruleMatch(session.getNegativeCount(), rule);
    }

    @Override
    public RuleConditionEnum getCondition() {
        return RuleConditionEnum.NEGATIVE_COUNT;
    }
}
