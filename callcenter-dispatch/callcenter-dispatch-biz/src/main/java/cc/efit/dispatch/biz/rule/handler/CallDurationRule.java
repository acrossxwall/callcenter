...
@Service
@Slf4j
public class CallDurationRule implements RuleHandler  {
    @Override
    public boolean handleRuleMatch(RuleInfo rule, DialogueProcessSession session, List<InteractiveRecord> list, int callDuration) {
        ...
        log.info("callId:{},通话时长规则匹配",callId);
        return ruleMatch(callDuration,rule);
    }

    @Override
    public RuleConditionEnum getCondition() {
        return RuleConditionEnum.CALL_DURATION;
    }
}
