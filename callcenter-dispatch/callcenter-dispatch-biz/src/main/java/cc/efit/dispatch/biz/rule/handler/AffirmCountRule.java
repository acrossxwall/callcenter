...
@Service
@Slf4j
public class AffirmCountRule implements RuleHandler {
    @Override
    public boolean handleRuleMatch(RuleInfo rule, DialogueProcessSession session, List<InteractiveRecord> list, int callDuration) {
        ...
        log.info("callId 触发肯定次数规则匹配开始:{}", callId);
        return ruleMatch(session.getAffirmCount(), rule);
    }

    @Override
    public RuleConditionEnum getCondition() {
        return RuleConditionEnum.AFFIRM_COUNT;
    }
}
