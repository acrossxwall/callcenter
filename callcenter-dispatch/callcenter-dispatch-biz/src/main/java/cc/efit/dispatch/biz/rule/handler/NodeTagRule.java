...
@Service
@Slf4j
public class NodeTagRule implements RuleHandler {
    @Override
    public boolean handleRuleMatch(RuleInfo rule, DialogueProcessSession session, List<InteractiveRecord> list, int callDuration) {
        ...
        log.info("callId 触发节点标签规则匹配开始:{}", callId);
        Set<String> nodeTagSet = session.getMatchNodeLabels();
        return nodeTagSet.contains(rule.value());
    }

    @Override
    public RuleConditionEnum getCondition() {
        return RuleConditionEnum.TRIGGER_NODE_TAG;
    }
}
