...
@Service
@Slf4j
public class KnowledgeRule implements RuleHandler {
    @Override
    public boolean handleRuleMatch(RuleInfo rule, DialogueProcessSession session, List<InteractiveRecord> list, int callDuration) {
        ...
        log.info("callId 触发知识库规则匹配开始:{}", callId);
        Set<Integer> matchKnowledge = session.getMatchKnowledgeIds();
        return matchKnowledge.contains(Integer.valueOf(rule.value()));
    }

    @Override
    public RuleConditionEnum getCondition() {
        return RuleConditionEnum.TRIGGER_KNOWLEDGE;
    }
}
