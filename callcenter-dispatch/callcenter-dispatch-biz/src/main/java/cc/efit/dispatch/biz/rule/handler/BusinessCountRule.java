...
@Service
@Slf4j
public class BusinessCountRule implements RuleHandler {
    @Override
    public boolean handleRuleMatch(RuleInfo rule, DialogueProcessSession session, List<InteractiveRecord> list, int callDuration) {
        ...
        log.info("callId 业务问题次数规则匹配开始:{}", callId);
        return ruleMatch(session.getBusinessKnowledge(), rule);
    }

    @Override
    public RuleConditionEnum getCondition() {
        return RuleConditionEnum.BUSINESS_QUESTION_COUNT;
    }
}
