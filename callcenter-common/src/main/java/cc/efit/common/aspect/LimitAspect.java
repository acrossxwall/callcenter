...
public class LimitAspect {
    ...
    @Pointcut("@annotation(cc.efit.annotation.Limit)")
    ...
    public Object around(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        if (StringUtils.isEmpty(key)) {
            if (limitType == LimitType.IP) {
                key = StringUtilsExternal.getIp(request);
            } else {
                key = signatureMethod.getName();
            }
        }

        List<String> keys = List.of(StringUtils.join(limit.prefix(), "_", key, "_", request.getRequestURI().replace("/","_")));
