...
public class OnlineUserService {
    ...
    public void save(JwtUserDto jwtUserDto, String token, HttpServletRequest request){
        String dept ;
        if (UserTypeEnum.SYSTEM_ADMINISTRATOR.getCode().equals(jwtUserDto.getUser().getUserType())) {
            dept = "系统管理员";
        }else if (UserTypeEnum.ORG_ADMINISTRATOR.getCode().equals(jwtUserDto.getUser().getUserType())) {
            dept = "租户管理员";
        }else{
            dept = jwtUserDto.getUser().getDept().getName();
        }
        String ip = StringUtilsExternal.getIp(request);
        String id = tokenProvider.getId(token);
        String browser = StringUtilsExternal.getBrowser(request);
        String address = StringUtilsExternal.getCityInfo(ip);
